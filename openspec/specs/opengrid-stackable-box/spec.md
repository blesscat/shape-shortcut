...
`fullBottomHoleGrid`, the enum `topRimMode`, the enum `bottomMode`,
...
`無角座`, `鎖定角座`, and `內建角座` respectively. `topRimMode` MUST be
exactly one of `stacking-rail` or `flat-top`, and `bottomMode` MUST be
exactly one of `stacking`, `thin-shell`, or `none`. `x` and `y` MUST be
...
mm and MUST represent clear internal box height. Height semantics MUST remain
unchanged within every top-rim and bottom-structure combination. The default snapshot
MUST be `x=2`, `y=2`, `height=20`,
`cornerSeatMode='detachable-corner-seat'`, `fullBottomHoleGrid=false`,
`topRimMode='stacking-rail'`, and `bottomMode='stacking'`.
`bottomMode='none'` and `fullBottomHoleGrid=true` MUST NOT be combined;
validation MUST reject that snapshot with a field-specific error.

The four opening triples MUST retain their current names, ranges, defaults, and
geometry semantics. The stackable-box panel MUST expose exactly one visible
radio group for the top rim with the labels `堆疊` (`stacking-rail`) and
`平頂` (`flat-top`), exactly one visible radio group for the bottom structure
with the labels `堆疊` (`stacking`), `薄殼` (`thin-shell`), and `無`
(`none`), and exactly one visible radio group for the locating seat with the
three labels above. The normalized `basePlateMode` and `thinShellMode`
fields MUST remain available to legacy or programmatic snapshots for
normalization, but MUST NOT become visible profile choices.

When a legacy snapshot contains `thinShellMode` or `basePlateMode`, hydration
MUST normalize them before validation: `thinShellMode=true` MUST map to
`topRimMode='flat-top'` and `bottomMode='thin-shell'`;
`thinShellMode=false` without `basePlateMode=true` MUST map to
`topRimMode='stacking-rail'` and `bottomMode='stacking'`; and
`basePlateMode=true` with `thinShellMode=false` MUST map to
`topRimMode='stacking-rail'` and `bottomMode='none'`. Canonical
`topRimMode` and `bottomMode` values MUST take precedence over stale legacy
booleans, and persistence MUST converge to the canonical fields after a valid
update.
...
- **AND** the existing OpenGrid footprint, height, top-rim, bottom-structure, and opening defaults
...
#### Scenario: Legacy mode booleans normalize to the new controls

- **WHEN** a persisted snapshot contains `thinShellMode=true` or
  `basePlateMode=true`
- **THEN** hydration MUST produce the corresponding `topRimMode` and
  `bottomMode` values defined in this requirement
- **AND** the panel MUST display the mapped `上緣` and `盒底` radio
  selections
- **AND** persistence MUST converge to the canonical fields after a valid
  update

#### Scenario: Open bottom and full grid are mutually exclusive

- **WHEN** a snapshot sets `bottomMode='none'` and `fullBottomHoleGrid=true`
- **THEN** validation MUST return a field-specific error
- **AND** the invalid snapshot MUST NOT replace the last valid revision

#### Scenario: Existing box geometry parameters remain valid

- **WHEN** a valid snapshot changes X/Y, height, top-rim mode, bottom mode,
...
Additional diagonal relief at every bottom cell corner MUST clear openGrid board junctions through the insertion depth while leaving the floor continuous. The remaining straight guide and bearing surfaces MUST retain the existing box-to-box seated height and sliding clearance; corner relief MAY terminate below the floor underside at the board seating datum.
...
- **WHEN** one `stacking-rail` + `stacking` box is placed above another such box with compatible footprints
...
- **AND** the bottom guide MUST use a 0.8 mm bed-facing 45° foot, a 1.8 mm vertical segment, and an internal seam slot that tapers at 19° from the 6.4 mm bed opening to a 4.5 mm opening at the floor underside without a horizontal closure land
...
- **WHEN** a single `stacking-rail` + `stacking` 1×1 box is placed on a `stacking-rail` + `stacking` 1×4 box
...
bottom assembly, the 2 mm thin-shell floor, and the open `none` bottom whose
floor is removed except for 2 mm corner pads at the four locating positions.
For `cornerSeatMode='none'`, it MUST generate no special corner locating geometry.
...
The stacking, thin-shell, and none bottoms MUST preserve their existing floor
and corner geometry while the detachable socket is cut from the bottom
interface; the `none` bottom MUST retain its 2 mm corner pads around every
active corner interface. The four nominal corner positions MUST continue to be
...
- **WHEN** a valid box with any top-rim and bottom-structure combination uses
  `cornerSeatMode='detachable-corner-seat'`
...
top-rim and bottom-structure combination, with ordinary holes generated only
where an active floor exists. In `detachable-corner-seat` mode it MUST validate every female socket
...
box-to-box interface contracts of every supported top-rim and bottom-structure
combination.
...
- **WHEN** a valid snapshot in any supported top-rim and bottom-structure
  combination uses any of the three seat modes
...
- **WHEN** a `stacking-rail` + `stacking` box uses `cornerSeatMode='integrated'`
...
- **WHEN** a valid opening with positive depth is generated in any top-rim and bottom-structure combination for any of `+X`, `-X`, `+Y`, or `-Y`
...
- **WHEN** a valid opening is generated in any top-rim and bottom-structure combination
- **THEN** the opening bottom MUST remain at or above the active floor boundary required by that combination
- **AND** the active floor thickness and top-rim profile MUST remain valid
- **AND** the opening MUST NOT cut into the stacking bottom guide, ordinary holes, corner sockets, corner pads, or any unselected lower feature
...
- **AND** the `stacking-rail` upper rail and `stacking` lower guide outside the selected opening span MUST retain their existing dimensions and mating clearance
- **AND** the `thin-shell` floor and `none` corner pads outside the selected opening span MUST retain their declared dimensions
...
typed normalized parameters. In addition to the existing X/Y, height, mode,
...
exports with different geometry cannot overwrite one another. The mode suffix
MUST be the concatenation of a bottom-structure part (`stacking` → empty,
`thin-shell` → `-thin-shell`, `none` → `-open-bottom`) followed by a top-rim
part (`stacking-rail` → empty, `flat-top` → `-flat-top`), so the six
combinations produce six distinct identities and the default `stacking-rail`
+ `stacking` combination keeps the existing unsuffixed name. Filenames MUST
NOT depend on raw input formatting, and opening fingerprints MUST retain their
existing behavior.

#### Scenario: Mode suffixes distinguish every combination

- **WHEN** two valid boxes differ only in `topRimMode` or `bottomMode`
- **THEN** their STEP and STL filenames MUST differ by the deterministic mode
  suffix
- **AND** no two of the six combinations may share one filename identity
...
footprint, height semantics, top-rim and bottom-structure semantics,
...
without replacing the visible `上緣`, `盒底`, and locating-seat radio groups. When
...
- **AND** the generated geometry and existing export identity MUST remain the same as the corresponding top-rim and bottom-structure combination

#### Scenario: The user enables Hex Mesh mode

- **WHEN** a valid stackable-box snapshot has `honeycombMode=true`
- **THEN** the panel MUST retain the existing X/Y, height, bottom-hole, top-rim, bottom-structure, seat, and four-direction opening controls
...
- **AND** eligible openings in every bottom mode that provides a floor MUST pass through the active floor so the Hex Mesh is visible from both floor faces
...
- **AND** the outer bottom frame, corner structural regions and pads, bottom guide, grid-seam reliefs, and active floor transitions MUST remain solid
...
- **AND** the `stacking-rail` + `stacking` box-to-box sliding guide and the non-stacking `thin-shell` and `none` bottoms MUST retain their existing contracts
...
- **AND** an open `none` bottom MUST NOT force a no-cell fallback merely because it provides no floor region
...
OpenGrid X/Y, height, opening, and full-grid controls plus exactly one visible
top-rim radio group with `堆疊` and `平頂`, exactly one visible
bottom-structure radio group with `堆疊`, `薄殼`, and `無`, and exactly one
visible locating-seat radio group with `無角座`, `鎖定角座`, and `內建角座`. The
panel MUST NOT expose `basePlateMode` or `thinShellMode` as selectable
profile choices, and MUST preserve the existing
latest-wins, preview, commit, STEP, and STL lifecycle. The Worker MUST validate
the canonical enum parameters, load the shared detachable references only for
...
### Requirement: Top rim and bottom structure combinations

The stackable-box model MUST generate every valid combination of
`topRimMode` (`stacking-rail`, `flat-top`) and `bottomMode` (`stacking`,
`thin-shell`, `none`) as one non-empty valid solid:

- `stacking-rail` MUST fuse the existing stepped top rail onto the box rim so
  the box can receive an upper box; `flat-top` MUST end in one continuous
  1.2 mm 45° top chamfer whose outer edge is at the external height and
  higher than its inner edge, without the stepped rail, a horizontal rim
  plane, or any detached rim fragment.
- `stacking` MUST keep the existing 5 mm bottom assembly with its bottom guide
  profile and internal grid-seam reliefs; `thin-shell` MUST use a flat 2 mm
  floor with the standard 1.2 mm side walls, a 1.5 mm 45° chamfer around the
  outside bottom perimeter, and no bottom assembly or seams;
  `none` MUST remove the floor entirely except for 2 mm corner pads at the
  de-duplicated locating positions when an active `cornerSeatMode` interface
  exists; with `cornerSeatMode='none'` the bottom MUST stay fully open.
...
#### Scenario: Generate each top-rim and bottom-structure combination

- **WHEN** each of the six `topRimMode` × `bottomMode` combinations is
  generated with any valid seat mode and zero or more valid openings
- **THEN** every result MUST be a previewable, exportable single valid solid
  centered on the existing footprint
- **AND** only `stacking-rail` results MUST contain the stepped top rail and
  only `stacking` results MUST contain the bottom guide and seams
- **AND** `none` results MUST contain no floor material outside the 2 mm
  corner pads

#### Scenario: Height datums per bottom structure

- **WHEN** a valid box with clear internal `height=H` is generated
- **THEN** the upper inner-rim datum MUST be at `5 + H` for the `stacking`
  bottom and at `2 + H` for the `thin-shell` and `none` bottoms
- **AND** the external height MUST add the rail height only when
  `topRimMode='stacking-rail'`

#### Scenario: Open bottom keeps only the locating interfaces

- **WHEN** a `none`-bottom box uses `cornerSeatMode='detachable-corner-seat'`
  or `cornerSeatMode='integrated'`
- **THEN** each de-duplicated corner position MUST retain its complete socket
  or seat interface on a 2 mm corner pad
- **AND** the floor between the corners MUST remain open through the full
  footprint interior

### Requirement: Freely removable board seating

The `opengrid-stackable-box` stacking bottom without protruding locating seats MUST fit matching whole-cell openGrid Lite and Full boards by vertical insertion and removal, without clips, undercuts or interference. Each cell corner, including internal junctions, MUST clear the board. The seated box MUST have positive bearing contact and lateral location while preserving the existing box-to-box sliding and stacking behavior, continuous floor, model identity and parameter defaults. Half-cell footprints and locating-seat modes MUST be explicitly checked; compatibility MUST NOT be inferred from whole-cell seat-free results.

#### Scenario: Whole-cell insertion and removal
- **WHEN** a seat-free stacking box of 1×1, 1×2 or 2×2 cells is placed on a matching Lite or Full board
- **THEN** it MUST reach a stable seated position without solid intersection and lift vertically without interference
- **AND** a small displacement below the seated position MUST intersect the board to demonstrate positive support
