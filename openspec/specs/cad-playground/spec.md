...
The playground MUST let the user edit the selected instance's parameters through a generic form generated from that component's parameter schema. Parameter input MUST reuse the component definition's validation; an invalid value MUST show the component's field diagnostic and MUST NOT be sent for generation. Editing parameters MUST NOT change other instances of the same component.
...
The playground MUST render every instance with a planning-grade proxy mesh generated by the CAD Worker from an envelope-level representation that omits fine details such as cell cutouts, fillets, holes, and text. All scene instances MUST render as proxy meshes; the playground MUST NOT generate or display full-detail meshes or edge-line overlays. Selection or hover MUST be indicated by material color emphasis only. Instances with identical `modelId` and parameters MUST share one rendered geometry, and a change to one instance's parameters MUST NOT regenerate the shared geometry used by unchanged instances.
