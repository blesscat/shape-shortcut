...
export function tissueBoxLayout(p: TissueBoxParameters) {
  ...
  const supportWidth = width - 2 * p.outerRadius
  ...
  return {
    ...
    plateHeight: Math.max(gridPitch, height * cosine),
...
export function tissueBoxSlotOrigins(p: TissueBoxParameters): TissueBoxPoint[] {
  const l = tissueBoxLayout(p)
  return Array.from({ length: l.columns }, (_, column) => [
    (column - (l.columns - 1) / 2) * OPENGRID_GRID_CONFIGURATION.fullPitch,
    0,
    OPENGRID_GRID_CONFIGURATION.fullPitch / 2,
  ])
}
...
export function validateTissueBoxParameters(
  ...
  if (p.slotLength < p.slotWidth + 2 || p.slotLength > slotLimits.length)
    issue('slotLength')
  if (issues.length) return { valid: false, issues }
  const bounds = tissueBoxBounds(p)
  if (bounds.max.some((max, index) => max - bounds.min[index]! > 500))
    issue('parameters')
